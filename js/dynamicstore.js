...
GDynamicStore = {
	...
	BuildBundleDescription: function( Bundle, $Description )
	{
		...
		else if ( Bundle.m_cUserItemsInBundle < Bundle.m_cTotalItemsInBundle )
		{
			...
			$Description.append( '<div>Compre o conjunto para economizar %1$s%% em %2$s itens que você ainda não possui!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cUserItemsInBundle ).replace( '%%', '%' ) );
		...
		{
			$Description.html( '<div>Compre o conjunto para economizar %1$s%% em %2$s itens!</div>'.replace( '%1$s', Bundle.m_nDiscountPct ).replace( '%2$s', Bundle.m_cTotalItemsInBundle ).replace( '%%', '%' ) );
		}
...
function UpdateStoreBannerForAdditionalCartDiscount( nCartDiscount )
{
	...
	<div class="placeHolder_lunarSale2019_giftActiveBar">	\
		...
		<div class="lunar_sale_title"><img src="https://store.akamai.steamstatic.com/public/images/promo/lunar2019/lny2019_title_pt-br.png"/></div> \
...
function BuildCreatorCapsuleToAppend( oItem )
{
	...
	$FollowLink.bind('updateFollowState', function () {
		...
		{
			$FollowLink.html( '<span><img src="https://store.akamai.steamstatic.com/public/images/v6/ico/ico_selected_green.png">Seguindo</span>' );
