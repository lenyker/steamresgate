...
GHomepage = {
	...
	BuildMainCapsuleItem: function( rgItem, strFeatureContext, nDepth )
	{
		...
		{
			$ImgCtn.append( $J('<div/>', {'class': 'broadcast_live_stream_icon' } ).append( 'Ao vivo') );
		}
		...
		if( rgRecommendationReasons.recommended_by_friend )
		{
			...
			var $ReasonMain = $J('<div/>').addClass('main').addClass('friend').html( "<strong>Recomendado<\/strong> por <span>%1$s<\/span>".replace("%1$s", friend.name ) );
		...
		else if( rgRecommendationReasons.recent_release_by_creator && rgRecommendationReasons.recent_release_by_creator.creator_info )
		{
			...
			var $ReasonLocal = creator.link.indexOf( 'developer/' ) >= 0 ? "<strong>Desenvolvimento<\/strong>: <br><span>%1$s<\/span>" : "<strong>Distribuidora<\/strong>: <br><span>%1$s<\/span>";
		...
		else if( rgRecommendationReasons.recommended && GDynamicStore.s_rgRecommendedTags )
		{
			...
			if( rgMatchedTags.length == 0 )
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "<strong>Recomendado<\/strong> por jogar jogos similares" );
			else
				var $ReasonMain = $J('<div/>').addClass('main').addClass('bytags').html( "<strong>Recomendado<\/strong> por jogar jogos com os marcadores" );
		...
		if( bShowAdditionalReasons )
		{
			...
			for( var key in rgRecommendationReasons )
			{
				...
				if( key == 'top_seller' )
					...
					strReason = "Lan\u00e7amento";
				else if( key == 'recommended')
					strReason = "Similar aos jogos que voc\u00ea joga";
				else
					continue;
				...
				$el.on('hover focus', function(){
					...
					else
					{
						...
						if ( rgData['review_summary'] )
						{
							...
							$elReviewData.append( $J('<div>', {'class': 'title'}).text( rgData.appids ? 'Média das análises de usuários para itens do conjunto:' : 'Análises de usuários:') );
						...
						if( rgData.tags )
						{
							...
							{
								var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/pt-br/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgData.tags[i] ) ), 'tab_preview' );
						...
						for( var i=0; i < 4; i++ )
						{
							...
							{
								var $elScreenshot = $J('<div>', {'class': 'screenshot empty'}).append("<div>").text("Esta captura de tela pode conter conte\u00fado adulto.");
								$elInfoDiv.append($elScreenshot);
							}
						}
	...
	RenderRecommendedByDeepDiveCarousel: function()
	{
		...
		focusedAppTitle = 'Porque você jogou <strong>%s</strong>'.replace( "%s", focusedAppTitle);
	...
	RenderCommunityRecommendations: function()
	{
		...
		{
			elemPrefLastUpdated.text( 'Usando preferências salvas em %1$s'.replace( "%1$s", GHomepage.strCommunityRecommendationsPrefLastSaved ) );
		}
	...
	FillPagedCapsuleCarousel: function( rgCapsules, $elTarget, fnCapsule, strNavContext, nCapsules )
	{
		...
		for( var j=0; j<rgCapsules.length; j+=nCapsules )
		{
			...
			for( var k=0; k < nCapsules && k + j < rgCapsules.length; k++ )
			{
				...
				{
					const panelString = '{"onOptionsActionDescription":"Ver tudo","onOptionsButton":"window.location=\'%1$s\'"}';
	...
	BuildHomePageGenericCap: function( strFeatureContext, unAppID, unPackageID, unBundleID, rgOptions, nDepth )
	{
		...
		{
			$CapCtn.append(
					$J('<div/>', {'class': 'broadcast_live_stream_icon' } ).append( 'Ao vivo')
			);
		}
	...
	BuildCommunityRecommendationCap: function( strFeatureContext, oItem )
	{
		...
		if ( numReviews > 1 )
		{
			...
			let strNumReviews = 'Análise %1$s de %2$s'.replace( "%1$s", 1 ).replace( "%2$s", numReviews );
			...
			arrowLeft.click( function() {
				...
				var strNumReviews = 'Análise %1$s de %2$s'.replace( "%1$s", curIdx + 1 ).replace( "%2$s", numReviews );
		...
		for( var i = 0; i < rgReviews.length; ++i)
		{
			...
			var $ViewMore = $J( '<div class="view_more"><a href="' + review.review_link + '" target="_blank">Ler análise completa</a></div>' );
			...
			if ( reviewer )
			{
				...
				{
					var strHours = '%s horas jogadas no momento da análise'.replace( "%s", v_numberformat( review.playtime_at_review / 60, 1 ) );
					$AuthorDetails.append( $J( '<div>', { class: "hours ellipsis", text: strHours } ) );
				}
				if ( review.votes_up > 0 )
				{
					var voteUpText = '1 pessoa achou esta análise útil';
					if ( review.votes_up > 1 )
					{
						voteUpText = '%2$s pessoas acharam esta análise útil'.replace( "%2$s", v_numberformat( review.votes_up ) );
					}
	...
	RenderRecommendedTags: function()
	{
		...
		for ( var i = 0; i < GDynamicStore.s_rgRecommendedTags.length; i++ )
		{
			...
			{
				var url = GStoreItemData.AddNavEventParamsToURL( 'https://store.steampowered.com/tags/pt-br/TAGNAME/'.replace( /TAGNAME/, encodeURIComponent( rgTag.name ) ), 'gutter' );
	...
	RenderMarketingMessages: function(  )
	{
		...
		var createMessageElementFunc = function( message, cssClass )
		{
			...
			if ( rgItemData )
			{
				if( rgItemData.has_live_broadcast )
				{
					$MessageCtn.append( $J('<div/>', {'class': 'broadcast_live_stream_icon' } ).append( 'Ao vivo') );
				}
...
CHomeSettings.prototype.DisplayPopup = function( $Btn )
{
	...
	this.m_$Popup.append( $J('<div/>', {'class': 'home_viewsettings_instructions' } ).text( 'Selecione os tipos de produtos que deseja ver nesta seção' ) );

	if ( this.m_ApplicableSettings.popular_new_releases )
		this.m_$Popup.append( this.RenderCheckbox( 'popular_new_releases', 'Lançamentos populares' ) );
	if ( this.m_ApplicableSettings.top_sellers )
		this.m_$Popup.append( this.RenderCheckbox( 'top_sellers', 'Mais vendidos' ) );
	if ( this.m_ApplicableSettings.early_access )
		this.m_$Popup.append( this.RenderCheckbox( 'early_access', 'Produtos em acesso antecipado' ) );
	if ( this.m_ApplicableSettings.games_already_in_library )
		this.m_$Popup.append( this.RenderCheckbox( 'games_already_in_library', 'Jogos que já estão na sua conta' ) );
	if ( this.m_ApplicableSettings.games_not_in_library )
		this.m_$Popup.append( this.RenderCheckbox( 'games_not_in_library', 'Jogos que não estão na sua conta' ) );
	if ( this.m_ApplicableSettings.recommended_for_you )
		this.m_$Popup.append( this.RenderCheckbox( 'recommended_for_you', 'Recomendados para você' ) );
	if ( this.m_ApplicableSettings.prepurchase )
		this.m_$Popup.append( this.RenderCheckbox( 'prepurchase', 'Produtos na pré-venda' ) );
	if ( this.m_ApplicableSettings.games )
		this.m_$Popup.append( this.RenderCheckbox( 'games', 'Jogos' ) );
	if ( this.m_ApplicableSettings.software )
		this.m_$Popup.append( this.RenderCheckbox( 'software', 'Softwares' ) );
	if ( this.m_ApplicableSettings.dlc_for_you )
		this.m_$Popup.append( this.RenderCheckbox( 'dlc_for_you', 'Conteúdo adicional p/ seus produtos' ) );
	if ( this.m_ApplicableSettings.recently_viewed )
		this.m_$Popup.append( this.RenderCheckbox( 'recently_viewed', 'Produtos vistos recentemente' ) );
	if ( this.m_ApplicableSettings.new_on_steam )
		this.m_$Popup.append( this.RenderCheckbox( 'new_on_steam', 'Novidades no Steam' ) );
	if ( this.m_ApplicableSettings.dlc )
		this.m_$Popup.append( this.RenderCheckbox( 'dlc', 'Conteúdo adicional' ) );
	if ( this.m_ApplicableSettings.video )
		this.m_$Popup.append( this.RenderCheckbox( 'video', 'Vídeos' ) );
	if ( this.m_ApplicableSettings.localized )
		this.m_$Popup.append( this.RenderCheckbox( 'localized', 'Jogos no meu idioma' ) );
	if ( this.m_ApplicableSettings.virtual_reality )
		this.m_$Popup.append( this.RenderCheckbox( 'virtual_reality', 'Realidade virtual' ) );
	if ( this.m_ApplicableSettings.recommended_by_curators )
		this.m_$Popup.append( this.RenderCheckbox( 'recommended_by_curators', 'Recomendado pelos seus Curadores Steam' ) );

	if ( this.m_ApplicableSettings.only_current_platform )
	{
		// this one is a little magic
		if ( GDynamicStore.s_bUserOnLinux )
			this.m_$Popup.append( this.RenderCheckbox( 'only_current_platform', 'Disponível para Linux/SteamOS' ) );
		else if ( GDynamicStore.s_bUserOnMacOS )
			this.m_$Popup.append( this.RenderCheckbox( 'only_current_platform', 'Disponível para macOS' ) );
	}
...
CHomeSettings.prototype.OnCheckboxToggle = function( strSettingName, $Checkbox )
{
	...
	$J.post( 'https://store.steampowered.com/dynamicstore/updatehomeviewsettings', {
		...
		ShowAlertDialog( 'Personalizar', 'Houve um problema ao salvar as suas preferências. Tente novamente mais tarde.' );
	} );
};
...
GSteamCurators = {
    ...
    BuildResponsiveHomePageGiantCap: function( strFeatureContext, oItem )
	{
        ...
        var $Status = $J('<div/>', {'class': 'recommendation_status recommended'}).text( "Recomendado" );
			 ...
			 GSteamCurators.rgAppsRecommendedByCurators.apps.length != 0 )
		{
			...
			if ( rgRecommendedApps.length >= 5 )
			{
				...
				for( var j=0; j<rgRecommendedApps.length; j++ in rgRecommendedApps )
				{
					...
					var $elButtonWishlist = $J('<span />').text("Adicionar \u00e0 lista de desejos");
