...
{
	if ( nCount == 0 )
		$JFromIDOrElement('wishlist_link').text( 'Lista de desejos' );
	else
		$JFromIDOrElement('wishlist_link').html( 'Lista de desejos (<span id="wishlist_item_count_value">' + nCount.toString() + '</span>)' );
}
...
function AddToWishlist( appid, divToHide, divToShowSuccess, divToShowError, navref, divToHide2 )
{
	if ( !g_AccountID )
	{
		ShowAlertDialog( "Inicie a sess\u00e3o...", "Inicie a sess\u00e3o no Steam para adicionar um item \u00e0 lista de desejos." )
		.done( function() {
		...
		.done( function( data ) {
			...
			if ( data && data.success ) {
				...
				if ( data.saleTaskCompleted ) {
					NewStickerPackModal( 'Adicione algo à lista de desejos' );
				}
			}
			else {
...
function IgnoreButton( button, appid )
{
	$J.post( 'https://store.steampowered.com/recommended/ignorerecommendation/', {
		...
		ShowAlertDialog( 'Ignorar aplicativo', 'Houve um problema ao salvar as suas alterações. Tente novamente mais tarde.' );
	});
}
		...
		$J.get( url, {ll: 'brazilian'} ).done( function( data ) {
			...
			if ( rgYourPopularTags.length > 0 )
			{
				...
				for( var i = 0; i < rgYourPopularTags.length && i < 5; i++ )
				{
					...
					var $Link = $J('<a/>', {'class': 'popup_menu_item', 'href': 'https://store.steampowered.com/tag/pt-br/' + encodeURIComponent( tag.name ) });
					$Link.text( tag.name );
					$Element.append( $Link );
				}
			}
...
function AddFreeLicense( subid, strDisplayName )
{
	...
	$J.when.apply( $J, posts ).done( function() {
		...
		if ( data && data.purchaseresultdetail == 9 )
			...
			ShowAlertDialog( strDisplayName, 'Para adicionar este produto à sua conta Steam, você precisa ter o produto base.' );
		else
			ShowAlertDialog( strDisplayName, 'Houve um problema ao adicionar este produto à sua conta. Tente novamente mais tarde.' );
	}).always( function () {
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	.done( function() {
		...
		Modal.Dismiss();
		ShowAlertDialog( 'Alterar idioma', 'Houve um problema ao salvar as suas alterações. Tente novamente mais tarde.' );
	});
}
...
function ShowGamePurchaseDropdown( elemLink, elemPopup )
{
	...
	if ( window.UseTabletScreenMode && window.UseTabletScreenMode() )
	{
		...
		var $Dialog = ShowDialog( 'Selecione uma opção de compra', $Content.show() );
		$Dialog.AdjustSizing();
	}
	else
	{
...
function FollowCuratorWithCallback( clanID, bFollow, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Inicie a sess\u00e3o", "Inicie a sess\u00e3o para seguir um curador");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Erro', 'Houve um problema ao tentar seguir o Curador Steam. Motivos possíveis:<ul><li>Não é possível seguir um curador ignorado por você.</li><li>Você só pode seguir até 100 curadores ao mesmo tempo</li><li>Contas limitadas não podem seguir um curador</li></ul>' );
		}
	);
	return false;
}
...
function IgnoreCuratorWithCallback( clanID, bIgnore, onComplete )
{
	...
	if ( !bHaveUser )
	{
		ShowAlertDialog("Inicie a sess\u00e3o", "Inicie a sess\u00e3o para ignorar um Curador Steam");
		return;
	}
	...
	$J.post(
		...
		{
			ShowAlertDialog( 'Erro', 'Houve um problema ao ignorar o Curador Steam.<br>Observação: Não é possível ignorar um curador que você segue.' );
		}
	);
	return false;
}
